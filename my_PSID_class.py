...
class mortgage_pd(pd.DataFrame):
    ...
    IN:
        ...
        only_first_n=10: in case there are too many categories, pick only the top n, and sum the rest as others. Default value is 10 (Note it also performs summation if we use mean of median)
        other_var=True: if true, and the number of categories is greater than only_first_n, include the 'Other' group in the plot
    ...
    """
    def my_plot(self,var_name,group_verti,group_horiz,method='sum',title=None,only_first_n=10,other_var=True,axis=None,legend_on=False,wid=0.6,figs=(7,5),stack=True,dic=None):
        ...
        only_first_n = min(only_first_n,len(var_by_group))
        ...
        if dic==None:
            ...
            out_dic = dic           
        remain_group = var_by_group.tail(len(var_by_group) - only_first_n)
        remain_total = remain_group.sum(axis=0)
        out_group = var_by_group.head(only_first_n).copy()
    ...
    def my_subplot(self,horiz,verti,super_title,out_path='',var_investigate='mortgage_in_thousands'):
        ...
        _,in_dic = self.my_plot(var_investigate,group_horiz=horiz,group_verti=verti, \
                                method='sum',only_first_n=10,title='Total',other_var=True,axis=axes[0,0],legend_on=False)
        self.my_plot(var_investigate,group_horiz=horiz,group_verti=verti, \
                                method='sum',only_first_n=10,title='Total Top Categories',other_var=False,axis=axes[0,1],legend_on=True,dic=in_dic)
        self.my_plot(var_investigate,group_horiz=horiz,group_verti=verti, \
                                method='count',only_first_n=10,title='Total Incidence',other_var=False,axis=axes[1,0],legend_on=False,dic=in_dic)
        self.my_plot(var_investigate,group_horiz=horiz,group_verti=verti, \
                                method='mean',only_first_n=10,title='Median',other_var=False,axis=axes[1,1],legend_on=False,stack=False,dic=in_dic)
        f.set_figheight(8)
        f.set_figwidth(10)
...
class mortgage_data(object):
    ...
    def mortgage_init(self):
        merge_data = mortgage_pd(pd.merge(self.ind_data,self.fam_data,how='left',on=['Year_Mortgage','Family_ID','SSN_deidentified']))
